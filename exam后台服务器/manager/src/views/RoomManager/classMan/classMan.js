...
function IndexPage(props) {
  let {getGrade,classData,subjectData,roomData,addGrande,delClass,gradUpdata}=props;
  const columns = [
    {
      title: '班级名称',
...
let mapDispatchToProps=dispatch=>{
    return {
       getGrade(){
         dispatch({
           type:"manager/gtade", //获取班级展示
         })
       },
       addGrande(payload){
        dispatch({
          type:"manager/addrade",
          payload,
        })
       },
       delClass(payload){
        dispatch({
          type:"manager/delClass",
          payload,
        })
       },
       gradUpdata(payload){
         dispatch({
           type:"manager/gradUpdata",
           payload,
         })
       }
    }
